...
if [ $# -eq 1 ]; then
    OUT=${NAME}.txt
    cd GPT/outputs && touch ${OUT}
    echo Running ${NAME}
    cd ../.. && python3 ${FILE} -ip ${IP} -u ${USER} -p ${PASS} > GPT/outputs/${OUT}
    echo Stored output in GPT/outputs/${NAME}.txt
else
    case "$FLG0" in
        ...
        *)
            ...
            if [ $# -gt 2 ]; then
                ...
                cd ../.. && python3 ${FILE} -ip ${IP} -u ${USER} -p ${PASS} ${FLGS}> GPT/outputs/${OUT}
            else
                echo Running ${NAME} with option ${FLG0} ...
                cd ../.. && python3 ${FILE} -ip ${IP} -u ${USER} -p ${PASS} ${FLG0} > GPT/outputs/${OUT}
            fi
            echo Stored output in GPT/outputs/${OUT}
            ;;
    esac
fi
