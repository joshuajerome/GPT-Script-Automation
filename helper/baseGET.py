...
def check_supported_idrac_version():
    if args["x"]:
        response = requests.get(f'{redfish_api_path}' % idrac_ip, verify=False, headers={'X-Auth-Token': args["x"]})
    else:
        response = requests.get(f'{redfish_api_path}' % idrac_ip, verify=False, auth=(idrac_username, idrac_password))
    data = response.json()
